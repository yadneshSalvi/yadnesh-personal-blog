// src/app/newsletter/sitemap.xml/route.ts
...
export function GET() {
  ...
  const entries: Entry[] = [
    { loc: absoluteUrl("/newsletter"), lastmod: hubLastMod, changefreq: "daily", priority: "0.9" },
    { loc: absoluteUrl("/newsletter/archive"), lastmod: hubLastMod, changefreq: "daily", priority: "0.7" },
    { loc: absoluteUrl("/newsletter/how-it-works"), lastmod: hubLastMod, changefreq: "monthly", priority: "0.6" },
  ];
  ...
  for (const entry of getTopics()) {
    ...
    entries.push({
      loc: absoluteUrl(`/newsletter/topics/${entry.topic}`),
      lastmod: hubLastMod,
      changefreq: "weekly",
      priority: "0.5",
    });
