...
function applyEvent(blocks: Block[], event: AgentEvent): Block[] {
  ...
  // Part 11: the inspection's card. "entered" opens it; each parsed
  // finding lands as a row; "exited" seals it with the counts and the
  // raw text. One review per turn, so the LAST review block is the one.
  if (event.type === "review_state") {
    if (event.phase === "entered") {
      return [...blocks, { type: "review", phase: "running", findings: [] }];
    }
    const i = blocks.findLastIndex((b) => b.type === "review");
    if (i < 0) return blocks;
    return blocks.map((b, j) =>
      j === i && b.type === "review"
        ? { ...b, phase: "done", counts: event.counts, raw: event.raw }
        : b,
    );
  }
...
function BlockView({
  ...
  if (block.type === "review")
    return <ReviewReportCard block={block} working={working} onFix={onFix} />;
...
function WorkingTimer({ startedAt, label }: { startedAt: number; label: string }) {
...
export default function Home() {
  ...
  // Part 12: the composer's pattern-book toggle. Sticky, not one-shot —
  // brand rules are a standing choice, unlike a single blueprint turn.
  // Armed only when the engine actually lists the skill.
  const [brandKit, setBrandKit] = useState(false);
  const [skills, setSkills] = useState<SkillInfo[]>([]);
  // Part 12: the health board rows (GET /mcp/servers).
  ...
  // Part 11: the road to /p/. `gate` holds the latest inspection's
  // counts and means "an inspection vouches for the site as it stands";
  // ANY later turn nulls it (mirroring the backend's staleness rule).
  // All of it is derived from the log, so replay rebuilds it — a fresh
  // tab knows the gate state without asking.
  const [gate, setGate] = useState<ReviewCounts | null>(null);
  ...
  // The engine-level surfaces: which pattern books exist, and how the
  // rented tools are doing. Pulled once at startup, again when the
  // panel opens, and on any mcp_status event that rides a stream.
  const loadEngine = useCallback(async () => {
    try {
      ...
      // A dead backend already speaks up elsewhere; the board can wait.
    }
  }, []);
  ...
  const handleEvent = useCallback(
    (event: AgentEvent, projectId: string) => {
      ...
      if (event.type === "session_start") {
        ...
        setPlan(null);
        setWorking(true);
        // Part 11, the gate's mirror: an inspection turn starts the
        // inspector's clock; ANY OTHER turn makes the last inspection
        // stale — the backend will 409 a publish, so the button disarms.
        if (event.review) {
          setReviewRunning(true);
          setReviewStartedAt(event.started_at_ms ?? Date.now());
        } else {
          setGate(null);
        }
        setMessages((all) => {
          ...
          if (event.message !== undefined)
            next.push({
              role: "user",
              text: event.message,
              blueprint: event.plan_first,
              brandKit: event.brand_kit,
            });
          next.push({
            role: "assistant",
            blocks: [],
      ...
      } else if (event.type === "review_state" || event.type === "review_finding") {
        // Block-shaped: the report card lives inside the open assistant
        // message like any card. The "exited" seal ALSO arms the gate —
        // the counts are exactly what the backend's gate will read.
        if (event.type === "review_state" && event.phase === "exited" && event.counts) {
          setGate(event.counts);
        }
        setMessages((all) => {
      ...
      } else if (event.type === "publish_state") {
        // The publish flow narrating itself. The manifest phases drive
        // the bar's progress state; a blocked or failed publish speaks
        // once, as a toast, and the bar goes back to telling the truth.
        if (event.phase === "manifest") setPublishing(true);
        if (event.phase === "blocked") {
          setPublishing(false);
          if (live) setToast(`Publish refused: ${(event.reasons ?? []).join(" · ")}`);
        }
        if (event.phase === "forced" && live)
          setToast("Publishing FORCED past the gate — this is on the record.");
        if (event.phase === "manifest_failed") {
          setPublishing(false);
          if (live) setToast(`The manifest failed twice: ${event.error ?? "unknown error"}`);
        }
        ...
        // A rented tool's power light flickered mid-turn. The board is
        // the truth; just re-read it (live only — replaying an old
        // flicker should not spam the endpoint).
        if (live) loadEngine();
    ...
    [loadFiles, refreshProjects, patchTurn, loadEngine],
  );
  ...
  // Kick off the inspection. The POST returns a claim ticket just like
  // /chat; everything after that — the entered/exited bookends, the
  // findings, the receipt — arrives on the stream, in every tab.
  const inspect = useCallback(async () => {
    if (!activeId) return;
    try {
      const res = await fetch(`${API_BASE}/projects/${activeId}/review`, {
        method: "POST",
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(
          typeof data?.detail === "string" ? data.detail : `The server said ${res.status}.`,
        );
      }
    } catch (err) {
      setToast(err instanceof Error ? err.message : "Could not start the inspection.");
    }
  }, [activeId]);

  // The earned button. The bar only arms it when the gate would pass,
  // but the backend re-checks anyway — a 409 here means another tab (or
  // a turn) changed the state first, and the reasons say so.
  const publishSite = useCallback(async () => {
    if (!activeId) return;
    setPublishing(true);
    try {
      const res = await fetch(`${API_BASE}/projects/${activeId}/publish`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    try {
      const res = await fetch(`${API_BASE}/projects/${activeId}/chat`, {
        ...
        body: JSON.stringify({
          ...
          effort,
          summary,
          // Part 12: sticky, and honest about steers — a mid-turn
          // message is absorbed as text; the skill only rides fresh
          // turns (the backend ignores it on the steer path).
          brand_kit: brandKit,
        }),
      });
      if (!res.ok) throw new Error(`The server said ${res.status}.`);
  ...
  // The inspector's findings, handed back as an ordinary build turn —
  ...
  // The composer's toggle arms only when the engine lists the skill —
  // an uninstalled pattern book is a disabled switch with instructions,
  // not a 409 surprise after typing a message.
  const brandKitSkill = skills.find((s) => s.name === "brand-kit" && s.enabled);
    ...
    <div className="flex h-dvh flex-col">
      <header className="flex items-center justify-between gap-3 border-b border-stone-200 px-5 py-3 dark:border-stone-800">
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          {/* The health board: the engine's rented tools (Part 12). */}
          <McpStatusPanel servers={mcpServers} onRefresh={loadEngine} />
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <section className="flex w-[44%] min-w-[360px] max-w-2xl flex-col border-r border-stone-200 dark:border-stone-800">
              ...
              {messages.map((message, i) =>
                message.role === "user" ? (
                  <div key={i} className="mb-5 flex flex-col items-end">
                    ...
                    {message.brandKit && (
                      <span
                        data-testid="brand-kit-chip"
                        className="mt-1 flex items-center gap-1.5 font-mono text-[11px] text-emerald-700 dark:text-emerald-400"
                      >
                        <span className="size-1.5 rounded-full bg-emerald-500" />
                        brand kit — skill attached to this turn
                      </span>
                    )}
                  </div>
                ) : message.role === "notice" ? (
                  ...
                  <div key={i} className="mb-6">
                    {message.blocks
                      // On an inspection turn the reviewer's closing
                      // prose repeats the findings verbatim; the report
                      // card already shows them (and keeps the raw text
                      // behind a fold), so the duplicate paragraph stays
                      // out of the transcript.
                      .filter((block) => !(message.review && block.type === "text"))
                      .map((block, j) => (
                        <BlockView
                          key={block.type === "text" || block.type === "review" ? j : block.id}
                          block={block}
                          working={working}
                          onDecide={decide}
                          onAnswer={answer}
                    ...
                    {message.status === "working" && message.startedAtMs !== undefined && (
                      <WorkingTimer
                        startedAt={message.startedAtMs}
                        label={message.review ? "Inspecting…" : "Building…"}
                      />
                    )}
                    {message.status !== "working" && (
                      <p className="mt-2 font-mono text-xs text-stone-400 dark:text-stone-500">
                        {[
                          ...
                          // Part 11: and an inspection names itself.
                          message.review ? "inspection (fresh eyes)" : null,
                          // Part 12: the receipt names the pattern book.
                          message.brandKit ? "brand kit" : null,
          ...
          {/* Part 11, the road to /p/: inspect → gate → publish. The
              bar derives everything from the log, so a second tab and a
              refreshed tab agree about whether the button is earned. */}
          <PublishBar
            hasSite={hasSite}
            working={working}
            ...
            publishedUrl={activeProject?.published?.url ?? null}
            onInspect={inspect}
          ...
          <footer className="border-t border-stone-200 px-5 py-4 dark:border-stone-800">
            ...
            <div className="mb-2.5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                {/* Part 12: the pattern book. Sticky — brand rules are a
                    standing choice — and armed only when skills/list
                    actually names the skill. */}
                <button
                  type="button"
                  role="switch"
                  aria-checked={brandKit}
                  data-testid="brand-kit-toggle"
                  disabled={!activeId || !brandKitSkill}
                  onClick={() => setBrandKit((v) => !v)}
                  title={
                    brandKitSkill
                      ? `Attach the ${brandKitSkill.name} skill to every message: ${brandKitSkill.description}`
                      : 'The brand-kit skill is not installed — cp -r skills/brand-kit "$CODEX_HOME/skills/"'
                  }
                  className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium transition-colors disabled:opacity-40 ${
                    brandKit
                ...
                >
                  <span
                    className={`size-1.5 rounded-full ${brandKit ? "bg-emerald-500" : "bg-stone-300 dark:bg-stone-600"}`}
                  />
                  Brand kit
                </button>
              </div>
