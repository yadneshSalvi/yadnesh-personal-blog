...
// A rented-tool approval (Part 12) shows the engine's own question: MCP
// calls are gated per call by elicitation, whatever the approvalPolicy.
  ...
  return (
    ...
    >
      ...
      {block.kind === "command" ? (
        ...
        // Part 12: the rented tool asks. The body is the engine's own
        // question, verbatim — it already names the server and the tool,
        // and inventing a prettier sentence invents a place to be wrong.
        <div className="px-4 py-3" data-testid="mcp-approval-body">
          ...
          {block.server && (
            <p className="mt-1.5 font-mono text-[11px] text-stone-400 dark:text-stone-500">
              MCP server: {block.server} · approvalPolicy does not cover rented tools —
              this card is the only gate
            </p>
          )}
        </div>
      ) : (
