...
// appear here without edits. mcpToolCall (Part 12) proves the point:
// the badge rendered before the frontend knew the kind existed; the
// only addition is cosmetic — name the rented tool the namespaced way
// the engine does (`openverse:search_images`), from the item detail.
export function ItemBadge({ block }: { block: ItemBlock }) {
  ...
  const label =
    block.kind === "fileChange" && files.length > 0
      ...
      : block.kind === "mcpToolCall" && block.detail.server
        ? `${block.detail.server}:${block.detail.tool}`
        : block.kind;
  return (
    <div className="my-1.5 flex max-w-xl items-center gap-2.5 rounded-lg border border-stone-200 bg-white px-3 py-2 dark:border-stone-800 dark:bg-stone-900">
      ...
      <span className="shrink-0 font-mono text-[11px] uppercase tracking-wider text-stone-400 dark:text-stone-500">
        {block.kind === "fileChange" ? "files" : block.kind === "mcpToolCall" ? "mcp" : "item"}
      </span>
    </div>
  );
}
