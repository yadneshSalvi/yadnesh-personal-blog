...
Three ways to extend the builder, in ascending cost, each one a
Pagewright feature rather than a tour:

Standing instructions. Every new workspace gets an AGENTS.md — the
site-rules poster (app.projects). The engine reads it from the thread's
cwd on its own; thread/start's `instructionSources` names it, and an
A/B build obeys it with no prompt changes. Rules that used to be
reviewer findings (alt text, no brief/ references) are now just how
this workshop builds.

Playbooks. The `brand-kit` skill (skills/brand-kit/SKILL.md, installed
into CODEX_HOME/skills) teaches reading a brief's assets folder and
applying logo, palette, and voice consistently. `skills/list` surfaces
it; a `{type: "skill", name, path}` input item on turn/start loads it
into exactly the turns that need it — the composer's "Brand kit"
toggle, on the wire.
...
stream as `mcpToolCall` items through the existing vocabulary. Two
truths shape the wiring (both verified live on 0.142.4):

- The MCP server lives outside the turn sandbox: once a call is
  allowed, `search_images` reaches the internet even with
  networkAccess: false. But SAVING a found image is the agent's own
  `curl` — governed by the same Part 6/7 grid as ever.
- EVERY MCP tool call asks permission first, and approvalPolicy does
  not enter into it: a `mcpServer/elicitation/request` server request
  (codex_approval_kind "mcp_tool_call") fires per call even under
  "never". Unanswered — or answered with the polite empty reply
  unhandled requests get — the engine scores it "user rejected MCP
  tool call" and the item fails in zero seconds, server never called.
  So the Part 7 inbox grows a third kind (see elicitation_handler).

GET /mcp/servers is the health board (app.mcp): status list merged
with startup notifications, because a server whose command never
launched is missing from the list, and the failure only exists in
the notification.

Everything Part 11 built — the inspector, the manifest, the gate —
...
from app import (approvals, eventlog, mcp, projects, publish, questions,
                 review, skills, turns)
...
async def lifespan(app: FastAPI):
    ...
    # Part 12: the seam's fourth customer. Every MCP tool call asks
    # permission through mcpServer/elicitation/request — regardless of
    # approvalPolicy (verified live: it fires under "never"). The Part 7
    # inbox answers it like any other ticket; only the response
    # vocabulary differs: accept/acceptForSession become
    # {"action": "accept"}, the session flavor via _meta persist
    # ("session" quiets this tool for the rest of this engine process;
    # "always" would write approval_mode = "approve" into config.toml's
    # per-tool table — which an operator can also declare up front).
    ...
    # Part 12: MCP startup status can arrive with threadId null (a
    # server booting belongs to the engine, not a conversation) — the
    # notification seam catches it for the health board.
...
class ChatRequest(BaseModel):
    ...
    # Part 12: attach the brand-kit skill to this turn — a
    # {type: "skill"} input item rides ahead of the message text.
...
async def chat(project_id: str, req: ChatRequest):
    ...
    # Part 12: the pattern book rides as an input item, ahead of the
    # words. The path is whatever skills/list said it is — if the skill
    # is not installed, the honest answer is a 409 with the fix, not a
    # turn that silently went out bare.
    input_items = []
    if req.brand_kit:
        ...
        if skill is None:
            raise HTTPException(
                status_code=409,
                detail="the brand-kit skill is not installed — "
...
async def mcp_servers():
    """The health board: every MCP server the engine knows, with its
    tool inventory and its latest startup state. A server whose command
    never launched shows up here as failed WITH the error — that row is
    this endpoint's whole reason to exist (see app.mcp)."""
    try:
        return {"servers": await mcp.list_servers(client)}
    except CodexError as exc:
...
async def skills_list():
    """The pattern books the engine can see (skills/list, flattened).
    The composer's toggle reads this to know whether brand-kit is
    installed — and shows the reader the path the engine found it at."""
    try:
        return {"skills": await skills.list_skills(client)}
    except CodexError as exc:
        raise HTTPException(status_code=502, detail=str(exc))
