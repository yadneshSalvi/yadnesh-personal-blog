...
Part 12 is the part that PROVES the vocabulary: an `mcpToolCall` item —
a whole new capability, a rented tool reaching the internet — flows
through `item_start`/`item_done` untouched and renders as the generic
badge; no new item lane was needed. The additions are one event and
one detail, plus a third kind on an old event:

- `mcp_status` — mcpServer/startupStatus/updated for notifications that
  ride a thread (a server starting or failing during a turn). The same
  notification with threadId null goes to app.mcp's store instead; the
  status panel reads the merged board from GET /mcp/servers either way.
- item_detail learns mcpToolCall: `{server, tool, status}` — so the wire
  says WHICH rented tool ran (`openverse:search_images`), even though
  the generic badge renders fine without it.
- `approval_request` gains kind "mcp_tool_call" (with `server` and the
  engine's own `message`): the elicitation that gates every MCP call
  rides the same approval events Part 7 built, and the same card
  answers it.

And session_start gains `brand_kit`, so every tab shows which turns
carried the pattern book.
...
def item_detail(item: dict) -> dict:
    ...
    if kind == "mcpToolCall":
        # The rented tool, named the namespaced way the engine names it.
        return {"server": item.get("server", ""),
                "tool": item.get("tool", ""),
...
def translate(note: dict) -> dict | None:
    ...
    if method == "mcpServer/startupStatus/updated":
        # A rented tool's power light, when the flicker happens during a
        # turn (threadId set — otherwise CodexClient's method handler
        # already caught it for app.mcp's board).
        return {"type": "mcp_status", "server": p.get("name", ""),
