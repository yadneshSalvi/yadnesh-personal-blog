...
class PendingApproval:
    ...
    def __init__(self, kind: str, thread_id: str, item_id: str) -> None:
        ...
        self.kind = kind  # "command" | "file_change" | "mcp_tool_call"
...
async def ask(kind: str, params: dict, notify) -> dict:
    ...
    await notify({"method": "approval/requested", "params": {
        ...
        # An MCP tool-call approval (Part 12) carries the server's name
        # and the engine's own question ("Allow the openverse MCP server
        # to run tool …?"); the other kinds carry neither.
