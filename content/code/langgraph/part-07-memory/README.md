...
- `page.tsx` — a `thread_id` minted with `crypto.randomUUID()` and kept in `localStorage`, a New conversation button, and a sidebar of past chats you can reopen
...
3. Click **New conversation** and ask again — the fresh thread has no idea. Two separate notebooks.
