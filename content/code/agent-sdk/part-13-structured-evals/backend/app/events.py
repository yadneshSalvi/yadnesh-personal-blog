...
New in Part 13, and LAST: complete can carry structured_output (the
validated summary the engine collected via the StructuredOutput call,
which gets no badge; the receipt tells its story) and stop_reason when
a budget or turn cap ended the run early. The vocabulary is finished.
Eleven parts of features rode one envelope, and the Part 3 parser is
still the parser.
"""
            ...
            if isinstance(message, SystemMessage) and message.subtype == "init":
                ...
                for block in message.content:
                    if isinstance(block, ToolUseBlock):
                        if block.name in CARD_BACKED_TOOLS | RECEIPT_BACKED_TOOLS:
                            hidden.add(block.id)  # a card or the receipt tells this story
                            continue
                        event = {
                            "type": "tool_use_start",
                            "tool_id": block.id,
