...
from app.events import clip, flatten
...
def history(workspace_id: str, session_id: str) -> dict:
    ...
    Same rules as the client's applyEvent, run over the past: text extends
    the turn, tool_use opens a block, tool_result completes it by id.
    Part 10 collects an old IOU: thinking blocks replay into the drawer,
    and the two card-backed tools replay as settled cards (the diary
    stores AskUserQuestion's input WITH the answers the bridge injected,
    so an old conversation shows what was asked and what you picked).
    """
    ...
    for m in get_session_messages(session_id, directory=str(workspace)):
        ...
        if m.type == "user":
                ...
                if block.get("type") == "tool_result" and turn:
                    for b in turn["blocks"]:
                        if b.get("type") == "tool_use" and b.get("id") == block.get("tool_use_id"):
                            ...
                            # Part 11: an Agent call's result (the subagent's
                            # report) is a content-block list; unwrap it the
                            # same way the live translator does.
                            b["result"] = clip(flatten(raw)) if raw is not None else ""
            ...
            for block in content or []:
                ...
                elif block.get("type") == "tool_use" and block["name"] == "AskUserQuestion":
                    answers = block.get("input", {}).get("answers")
                    turn["blocks"].append({
                        "type": "question", "id": block["id"],
                        "questions": block.get("input", {}).get("questions", []),
                        "status": "answered" if answers else "unanswered",
                        "answers": answers,
                    })
                elif block.get("type") == "tool_use" and block["name"] == "ExitPlanMode":
                    turn["blocks"].append({
                        "type": "plan", "id": block["id"],
                        "markdown": block.get("input", {}).get("plan", ""),
                        "settled": True,  # history never offers live buttons
                    })
                elif block.get("type") == "tool_use" and block["name"] == "StructuredOutput":
                    # Part 13: the diary records the summary as the tool
                    # call that collected it. Replay it where the live
                    # stream puts it: on the turn, not in the transcript.
                    turn["summary"] = block.get("input")
