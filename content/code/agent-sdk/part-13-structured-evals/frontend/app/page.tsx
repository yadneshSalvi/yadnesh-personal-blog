...
import type { AgentEvent, Answers, Artifact, Block, ChatMessage } from "@/lib/types";
import {
  answerQuestion,
  API_BASE,
  cancelChat,
...
function applyEvent(blocks: Block[], event: AgentEvent): Block[] {
  ...
  // Part 10: the negotiating parcels. Thinking accumulates like text but
  // into its own block; questions live the approval lifecycle; a plan
  // arrives already settled server-side and just needs rendering.
...
export default function Home() {
  ...
  // Part 13: the prepaid meter, as the user sets it. Kept as a string so
  // the field can sit empty ("off") without number-input fighting.
  ...
  async function handleAnswer(id: string, answers: Answers) {
    try {
      await answerQuestion(id, answers);
      // Same discipline as approvals: no local flip. The server emits
      // question_resolved and the card settles when the parcel arrives.
    } catch (err) {
      setToast({ message: (err as Error).message });
    }
  }
  ...
  function handleEvent(event: AgentEvent, es: EventSource) {
    if (event.type === "complete") {
      // A budget stop can end the run between a tool call and its result;
      // settle any badge still spinning so the turn reads as finished.
      setMessages((all) => {
        const last = all[all.length - 1];
        if (last?.role !== "assistant") return all;
        const blocks = last.blocks.map((b) =>
          b.type === "tool_use" && !b.done
            ? { ...b, done: true, result: b.result ?? "(the run ended before this call returned)" }
            : b,
        );
        return [...all.slice(0, -1), { ...last, blocks }];
      });
      patchLastTurn({
        status: stopRequestedRef.current ? "stopped" : "done",
        costUsd: event.total_cost_usd ?? undefined,
        durationMs: event.duration_ms,
        // Part 13: the receipt grew a form and a stop story. Both are
        // optional fields on the same old parcel; nothing else changed.
        summary: event.structured_output,
  ...
  async function send(text: string, modeOverride?: "ask" | "plan") {
    ...
    setMessages((all) => [
      // Any new message settles old plan cards: their buttons described
      // a decision that has now been taken (or overtaken).
      ...all.map((m) =>
        m.role === "assistant"
          ? { ...m, blocks: m.blocks.map((b) => (b.type === "plan" ? { ...b, settled: true } : b)) }
          : m,
      ),
    ...
    try {
      ...
      const ticket = await startChat(
        question,
        workspaceId,
        sessionId,
        runMode,
        thinking,
        Number.isFinite(budgetUsd) && budgetUsd > 0 ? budgetUsd : null,
      );
        ...
        <div className="flex min-w-0 flex-1 flex-col">
                    ...
                    {message.blocks.map((block, j) =>
                      block.type === "text" ? (
                        ...
                        <ThinkingDrawer
                          key={j}
                          block={block}
                          live={message.status === "working" && j === message.blocks.length - 1}
                        />
                      ) : block.type === "approval" ? (
                        <ApprovalCard key={block.id} block={block} onDecide={handleDecision} />
                      ) : block.type === "question" ? (
                        <QuestionCard key={block.id} block={block} onAnswer={handleAnswer} />
                      ) : block.type === "plan" ? (
                        <PlanCard
                          key={block.id}
                          block={block}
                    ...
                    {(message.status === "stopped" ||
                      ...
                      <p className="mt-2 font-mono text-xs text-stone-400 dark:text-stone-500">
                        {[
                          ...
                          message.stopReason === "max_budget" ? "budget exhausted" : null,
                          message.stopReason === "max_turns" ? "turn cap reached" : null,
          ...
          <footer className="border-t border-stone-200 px-5 py-4 dark:border-stone-800">
            ...
            <div className="mx-auto mb-2 flex w-full max-w-3xl items-center gap-2">
              <div
                className="flex overflow-hidden rounded-lg border border-stone-200 dark:border-stone-800"
                role="group"
              ...
              >
                {(["ask", "plan"] as const).map((m) => (
                  ...
                  >
                    {m === "ask" ? "Answer" : "Plan first"}
                  </button>
                ))}
              </div>
              <label className="flex items-center gap-1 rounded-lg border border-stone-200 px-2.5 py-1 font-mono text-[11px] uppercase tracking-wider text-stone-400 dark:border-stone-800 dark:text-stone-500">
                Budget $
                <input
                  value={budget}
                  ...
                  placeholder="off"
                  inputMode="decimal"
                  aria-label="Per-request budget in dollars"
                  className="w-10 bg-transparent normal-case text-stone-600 outline-none placeholder:text-stone-300 dark:text-stone-300 dark:placeholder:text-stone-600"
                />
              </label>
              <button
                type="button"
                ...
                className={`rounded-lg border px-2.5 py-1 font-mono text-[11px] uppercase tracking-wider ${
                  thinking
                    ? "border-accent bg-accent/10 text-accent"
                    : "border-stone-200 text-stone-400 hover:text-stone-600 dark:border-stone-800 dark:text-stone-500 dark:hover:text-stone-300"
                }`}
              >
              ...
              {mode === "plan" && (
                <span className="text-[12px] text-stone-400 dark:text-stone-500">
                  Read-only: the analyst proposes before it touches anything.
                </span>
              )}
            </div>
            <form
