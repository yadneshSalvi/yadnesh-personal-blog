"""Part 11: the analyst grows a team.

Two additions, and both are options rather than machinery. A reviewer
subagent (agents= plus the Task tool) gives the analyst a colleague
with fresh eyes and a strictly read-only toolbox, so key numbers can be
re-derived by someone who never watched them being computed. And skills
(a SKILL.md installed on every desk, discovered via
setting_sources=["project"] plus the Skill tool) hand it the house
report format on demand instead of stuffing it into every prompt.
"""
