...
export function toolLabel(block: ToolBlock): string {
  ...
  switch (block.name) {
    ...
    default: {
      ...
      // Part 12: the external fetch server. Show the host, not the
      // whole URL; the expanded badge and the card carry the rest.
      if (mcp?.[1] === "fetch" && mcp?.[2] === "fetch") {
        try {
          return `Fetching ${new URL(str(input, "url")).hostname}`;
        } catch {
