"""Part 12: the wider world, behind walls.

Two additions, and again both are options rather than machinery. An
external MCP server (mcp-server-fetch, a stdio subprocess the SDK
spawns and speaks the protocol to) gives the analyst the open web
through one gated tool. And the sandbox (beta) puts OS-enforced walls
around the Bash tool: writes outside the desk die, and new network
hosts leave through a proxy that asks the same can_use_tool gate
before letting a connection through.
"""
