...
<Figure
  ...
  caption="In-memory state, taken literally. A goldfish has a famous nine-second memory; a bot whose only memory is one request is worse. The PROD label is the foreshadowing for Part 8."
  width={1536}
  height={1024}
/>
...
<Quiz
  questions={[
    {
      q: "Why did the Part 6 bot forget your name between two messages?",
      options: [
    ...
    },
    {
      ...
      answer: 1,
      explain: "On each turn the checkpointer reloads the thread's saved messages, the `add_messages` reducer from Part 3 appends your one new message, and the grown list is saved again. You supply the new line; the graph supplies the rest.",
    },
    {
      ...
      answer: 2,
      explain: "`InMemorySaver` stores threads in a plain dict inside the running process. It's perfect for development and zero-setup, but a restart erases everything. That's the goldfish bowl labeled PROD, and swapping in `SqliteSaver` or `PostgresSaver` is the durable fix Part 8 points to.",
    },
  ]}
/>
