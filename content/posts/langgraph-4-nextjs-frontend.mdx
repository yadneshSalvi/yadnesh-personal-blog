...
<Figure
  ...
  caption="Today's destination. The same backend from Part 3, now answering you in a real chat UI instead of a terminal. By the end of this page, you built this."
  width={1120}
  height={639}
/>
...
React's way of giving a component memory. `useState(initialValue)` hands back a pair: the current value and a setter to change it, which is why you write it as `const [input, setInput] = useState("")`. Call the setter and React re-renders the component with the new value, so what's on screen always matches the state. Without it, a component would forget everything the moment it re-draws. You'll use one for every moving piece of this UI: the message list, the input box, and soon a loading flag.
</Callout>
...
The rule of thumb: add it the moment a component needs to *react* to something in the browser. State (`useState`), lifecycle (`useEffect`), and event handlers (`onClick`, `onSubmit`) all require it. A component that just renders static markup doesn't. Forget it and you get the exact error above, which is why we met it on purpose: now it's a familiar face, not a wall.
</Callout>
...
Because the URL changes. On your laptop the backend is `localhost:8000`; in Part 8, deployed, it's a public address. Reading it from `NEXT_PUBLIC_API_BASE_URL` means you change one line in one env file instead of hunting through your code. The `NEXT_PUBLIC_` prefix, from Part 1, is what makes the value visible to browser code in the first place. A backend URL is safe to expose; your API key, which has no such prefix, is not.
</Callout>
...
<Figure
  ...
  caption="The whole trip. Same path as your Part 2 curl, except a browser makes it and React paints the answer. The backend is the Part 3 one-node graph, untouched."
  width={1180}
  height={560}
/>
...
<Quiz
  questions={[
    ...
    },
    {
      ...
      answer: 1,
      explain: "Deployed in Part 8, the backend lives at a public address, not localhost. Reading it from an env file means a one-line change. The `NEXT_PUBLIC_` prefix is also what makes the value readable by browser code at all.",
    },
    {
      ...
      answer: 2,
      explain: "Browsers block cross-origin calls unless the server opts in; `:3000` and `:8000` are different origins. You added that opt-in back in Part 2, on purpose, for exactly this request. curl never enforced the check, which is why your terminal tests didn't need it.",
    },
    {
