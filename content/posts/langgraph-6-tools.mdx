...
<Figure
  ...
  caption="The same wire from Part 5, carrying two new labels. tool_start and tool_end interleave with the token envelopes; the blank-line framing and the done envelope are exactly as you left them."
  width={1040}
  height={620}
/>
...
<Quiz
  questions={[
    ...
    },
    {
      ...
      answer: 2,
      explain: "Because Part 5 wrapped every message in a `{type, ...}` envelope, the reader already parses one shape and branches on `type`. Tool events are just two more `type` values, so the blank-line buffering and JSON parsing carry over unchanged.",
    },
    {
